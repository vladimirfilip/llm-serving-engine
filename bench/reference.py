...
def generate(model, prompts: list[dict], new_tokens: int, long_new_tokens: int,
    ...
    short = [p for p in prompts if p.get("kind") != "long"]
    long = [p for p in prompts if p.get("kind") == "long"]
...
def prompt_hash(prompt: dict, new_tokens: int, long_new_tokens: int, dtype: str) -> str:
    """Identifies a cached generation: the prompt, how far it was generated, and in what dtype."""
    key = json.dumps([prompt["prompt_token_ids"], prompt.get("kind"), new_tokens,
                      long_new_tokens, dtype])
...
def task_generate(model_path: str, prompts: list[dict], out_path: str, new_tokens: int,
    ...
    def key(p: dict) -> str:
        return prompt_hash(p, new_tokens, long_new_tokens, dtype)

    missing = [p for p in prompts if cached.get(p["id"], {}).get("prompt_hash") != key(p)]
    if missing:
        ...
        for record in generate(model, missing, new_tokens, long_new_tokens, pad_id):
            ...
            cached[record["id"]] = record | {"prompt_hash": key(by_id)}
